...
test("buildTaskPacket appends SSoT injection for DAO workflow queries", async () => {
  const originalEnv = process.env.ARBITER_DOCS_INDEX_PATH;
  ...
  process.env.ARBITER_SSOT_PASSWORD = "taskpacket-ssot-password";

  try {
    const packet = await buildTaskPacket({
      id: "TASK-DAO",
      query: "Update DAO roster and assignment board"
    });
    assert.match(packet.contextPack, /## SSoT Injection/i);
    assert.match(packet.contextPack, /\[SSOT:dao-governance@/i);
    assert.deepEqual(packet.citations, []);
  } finally {
    if (originalEnv === undefined) {
      delete process.env.ARBITER_DOCS_INDEX_PATH;
