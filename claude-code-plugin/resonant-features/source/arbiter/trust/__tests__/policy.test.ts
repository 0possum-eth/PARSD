...
test("plugin paranoia mode blocks prompt injection patterns in tool args", async () => {
  const plugin = await ArbiterOsPlugin();
...
test("plugin system transform injects SSoT from user input and purges older duplicate versions", async () => {
  const originalRole = process.env.ARBITER_ROLE;
  const originalPassword = process.env.ARBITER_SSOT_PASSWORD;
  const originalCwd = process.cwd();
  const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "arbiter-plugin-ssot-"));
  process.env.ARBITER_ROLE = "arbiter";
  process.env.ARBITER_SSOT_PASSWORD = "plugin-ssot-password";
  ...
  try {
    ...
    await plugin["experimental.chat.system.transform"](
      {
        message: {
          role: "user",
          content: "Please validate ledger receipts and continuity"
        }
      },
      output
    );
    ...
    if (originalPassword === undefined) {
      delete process.env.ARBITER_SSOT_PASSWORD;
    } else {
      process.env.ARBITER_SSOT_PASSWORD = originalPassword;
    }
    await fs.promises.rm(tempDir, { recursive: true, force: true });
  }
});
