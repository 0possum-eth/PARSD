...
import { runM2Agent } from "../../arbiter/memory/m2Agent";
import { assertShieldSafe } from "../../arbiter/resonant/shield";
import { applySsotSystemTransform } from "../../arbiter/resonant/ssot";
...
export const ArbiterOsPlugin = async () => ({
  "experimental.chat.system.transform": async (input, output) => {
    ...
    try {
      await applySsotSystemTransform(input, output, { role: getRoleFromEnv() });
    } catch {
  ...
  "experimental.session.compacting": async () => ({
    summary: await (async () => {
      ...
      const memorySummary = await buildCompactionSummary({
