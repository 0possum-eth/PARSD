...
import { buildSsotAppendixForTaskPacket } from "../resonant/ssot";
...
export async function buildTaskPacket(task: TaskPacketInput): Promise<TaskPacket> {
  ...
  try {
    const ssotAppendix = await buildSsotAppendixForTaskPacket({
      query,
      role: process.env.ARBITER_ROLE
    });
    if (ssotAppendix.trim().length > 0) {
      pack = `${pack}\n\n${ssotAppendix}`;
    }
  } catch {
    // Keep packet construction resilient when SSoT docs are unavailable.
  }
